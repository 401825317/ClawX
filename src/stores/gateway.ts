...
interface GatewayState {
  ...
  init: () => Promise<void>;
  /** Read current Main-process state instead of relying on renderer events. */
...
/** A running process is not usable until its RPC transport has connected. */
export function isGatewayReadyForChatSend(status: GatewayStatus): boolean {
  // Older Main processes do not report gatewayReady, so only an explicit false
  // blocks a compatible client.

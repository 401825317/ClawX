...
const OPENAI_PATCH_MARKER = 'UCLAW_VIDEO_MODEL_VALIDATION_OPENAI_V2';
const PREVIOUS_OPENAI_PATCH_MARKER = 'UCLAW_VIDEO_MODEL_VALIDATION_OPENAI_V1';
...
const OPENAI_HELPER_ANCHOR = `function toBlobBytes(buffer) {`;

const OPENAI_HELPER_PATCH = `const ${OPENAI_PATCH_MARKER} = true;
function isSupportedOpenAIVideoGenerationModel(model, cfg) {
\treturn model === DEFAULT_OPENAI_VIDEO_MODEL || model === "sora-2-pro" || Boolean(resolveUClawManagedVideoModelCapability(cfg, model));
}
function toBlobBytes(buffer) {`;

const PREVIOUS_OPENAI_HELPER_PATCH = `const ${PREVIOUS_OPENAI_PATCH_MARKER} = true;
...
const CURRENT_OPENAI_HELPER_PATCH = `const ${OPENAI_PATCH_MARKER} = true;
function isSupportedOpenAIVideoGenerationModel(model, cfg) {
\treturn model === DEFAULT_OPENAI_VIDEO_MODEL || model === "sora-2-pro" || Boolean(resolveUClawManagedVideoModelCapability(cfg, model));
}`;
...
export function patchOpenClawOpenAiVideoModelValidationContent(content, filePath = '<memory>') {
  ...
  if (content.includes(PREVIOUS_OPENAI_PATCH_MARKER)) {
    return {
      content: replaceUnique(
        content,
        PREVIOUS_OPENAI_HELPER_PATCH,
        CURRENT_OPENAI_HELPER_PATCH,
        'previous OpenAI video model validation helper',
        filePath,
      ),
      changed: true,

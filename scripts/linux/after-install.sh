...
# Post-installation script for UClaw on Linux
...
# Create symbolic link for the UClaw app binary.
if [ -x /opt/UClaw/clawx ]; then
    ln -sf /opt/UClaw/clawx /usr/local/bin/uclaw 2>/dev/null || true
    ln -sf /opt/UClaw/clawx /usr/local/bin/clawx 2>/dev/null || true
elif [ -x /opt/UClaw/uclaw ]; then
    ln -sf /opt/UClaw/uclaw /usr/local/bin/uclaw 2>/dev/null || true
elif [ -x /opt/ClawX/clawx ]; then
    ln -sf /opt/ClawX/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/UClaw/resources/cli/openclaw"
[ -f "$OPENCLAW_WRAPPER" ] || OPENCLAW_WRAPPER="/opt/ClawX/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/UClaw/chrome-sandbox' 2>/dev/null || chmod 4755 '/opt/ClawX/chrome-sandbox' || true
else
    chmod 0755 '/opt/UClaw/chrome-sandbox' 2>/dev/null || chmod 0755 '/opt/ClawX/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/UClaw/resources/apparmor-profile'
    [ -f "$APPARMOR_PROFILE_SOURCE" ] || APPARMOR_PROFILE_SOURCE='/opt/ClawX/resources/apparmor-profile'
...
echo "UClaw has been installed successfully."

...
const SOURCE_DIR = path.join(ROOT, 'tools', 'portable-repair');
const OUTPUT_ROOT = path.join(ROOT, 'resources', 'bin');

const TARGETS = {
  'win32-x64': { goos: 'windows', goarch: 'amd64', fileName: 'UClawRepair.exe' },
};
...
function selectedTargets() {
  ...
  if (!TARGETS[target]) {
    throw new Error(`Unsupported portable repair target: ${target}`);
  }
  return [target];
}
...
function buildTarget(target) {
  ...
  const ldflags = target.startsWith('win32-') ? '-s -w -H=windowsgui' : '-s -w';
  ...
  console.log(`[build-portable-repair] Built ${path.relative(ROOT, outputPath)}`);
}
...
try {
  ...
  console.error(`[build-portable-repair] ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
}

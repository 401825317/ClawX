...
async function readJsonFile(filePath, label) {
  try {
    ...
    throw new Error(
      `${label} is missing or invalid: ${filePath} (${error instanceof Error ? error.message : String(error)})`,
    );
  }
}
...
async function assertPluginRuntimeDependencies(pluginDir, pkg, label) {
  ...
  for (const dependencyName of dependencies) {
    const packagePath = path.join(pluginDir, 'node_modules', ...dependencyName.split('/'), 'package.json');
    if (!(await fileExists(packagePath))) missing.push(dependencyName);
  }
  if (missing.length > 0) {
    throw new Error(`${label} is missing runtime dependencies: ${missing.join(', ')}`);
  }
}

async function assertBundledPluginMetadata(pluginsRoot, plugin) {
  const pluginDir = path.join(pluginsRoot, plugin.pluginId);
  const pkg = await readJsonFile(path.join(pluginDir, 'package.json'), `Plugin ${plugin.pluginId} package.json`);
  const manifest = await readJsonFile(
    path.join(pluginDir, 'openclaw.plugin.json'),
    `Plugin ${plugin.pluginId} manifest`,
  );
  ...
  for (const entry of declaredEntries) {
    if (await fileExists(path.join(pluginDir, entry))) {
      hasEntrypoint = true;
      break;
    }
  }
  ...
  await assertPluginRuntimeDependencies(pluginDir, pkg, `Plugin ${plugin.pluginId}`);
}

async function assertLocalPluginMetadata(pluginsRoot, pluginId) {
  const pluginDir = path.join(pluginsRoot, pluginId);
  const pkg = await readJsonFile(path.join(pluginDir, 'package.json'), `Plugin ${pluginId} package.json`);
  const manifest = await readJsonFile(path.join(pluginDir, 'openclaw.plugin.json'), `Plugin ${pluginId} manifest`);
  ...
  if (!pkg.main || pkg.main !== manifest.entry || !(await fileExists(path.join(pluginDir, manifest.entry)))) {
    throw new Error(
  ...
  await assertPluginRuntimeDependencies(pluginDir, pkg, `Plugin ${pluginId}`);
}
...
async function invokeHostApi(page, moduleName, actionName, payload) {
  return await page.evaluate(async (request) => {
    const api = window.clawx;
    if (!api?.hostInvoke) {
      return { ok: false, error: { message: 'Typed Host API is unavailable' } };
    }
    return await api.hostInvoke({
      id: `nsis-validation-${Date.now()}`,
      module: request.module,
      action: request.action,
      payload: request.payload,
    });
  }, { module: moduleName, action: actionName, payload });
}

function unwrapHostApiResponse(response) {
  const outer = response && typeof response === 'object' ? response : {};
  if (outer.ok === true) {
    return { ok: true, json: outer.data, error: '' };
  }
  return {
    ok: false,
    json: null,
    error: outer.error?.message || outer.error || 'Typed Host API returned an unsuccessful response',
  };
}
...
async function launchInstalledApp(installerRoot, sandboxRoot, gatewayPort, hostApiPort) {
  ...
  try {
    ...
    let lastError = '';
    while (Date.now() < deadline) {
      const response = unwrapHostApiResponse(await invokeHostApi(page, 'gateway', 'status'));
...
async function validatePackagedContents(installerRoot, sourcePackage, sourceCommit, requireCleanSource) {
  ...
  const pluginsRoot = path.join(installerRoot, 'resources', 'openclaw-plugins');
  for (const plugin of BUNDLED_OPENCLAW_PLUGINS) {
    await assertBundledPluginMetadata(pluginsRoot, plugin);
  }
  for (const pluginId of LOCAL_OPENCLAW_PLUGIN_IDS) {
    await assertLocalPluginMetadata(pluginsRoot, pluginId);
  }

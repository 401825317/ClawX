...
JunFeiAI 管理インストールでは、起動時に旧 `lingzhiwuxian/*` チャット参照を `openai/*` へ自動移行し、OpenClaw のネイティブ Responses アダプターを使用します。個人 OpenAI 設定の所有元を確認できない場合は上書きせず、プロバイダー設定で明示的に処理できます。旧 provider は互換経路として保持され、`xhigh` 推論と現在の Agent/永続タスク実行系も維持されます。出力開始前に `/responses` が 404 を返した場合だけ Chat Completions に一度フォールバックします。
JunFeiAI 管理インストールでは、Gateway 起動前に旧 `lingzhiwuxian/*` チャット参照を `openai/*` へ自動的に収束させ、管理対象の 372K コンテキストと `xhigh` 推論を備えた OpenClaw ネイティブ Responses アダプターを使用します。同じ冪等ブートストラップが最初の Agent ターン前に画像・動画 Provider と既定モデルも準備します。個人 OpenAI エンドポイントは上書きせず、その場合は旧 Chat Completions 経路と明示的な移行操作を残します。出力開始前に `/responses` が 404 を返した場合だけ Chat Completions に一度フォールバックします。

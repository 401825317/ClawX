...
会話レンダラーは、OpenClaw と Gateway の事実を単一のイベント駆動線形 Timeline として投影します。時系列で隣接する互換性のあるツール呼び出しはグループ化され、詳細は必要なときだけ読み込まれます。ユーザー向けに表示可能な thinking は既定で折りたたまれ、artifact、検証結果、承認、サブタスクは独立した Timeline 項目として表示されます。実行グラフはメインフローに常駐せず、必要なときだけ **実行詳細** から開きます。raw canonical event は開発者診断に限定されます。長い履歴は仮想化され、上へスクロールすると自動追従が解除されるため、ストリーミング更新によって表示位置が末尾へ強制的に戻されることはありません。Phase 5 では legacy/shadow レンダラーと runtime rollout flag を削除しました。ロールバックはコードまたはバージョンの復元で行い、transcript や保存済み会話の移行は不要です。

...
会話レンダラーは、OpenClaw と Gateway の事実を単一のイベント駆動線形 Timeline として投影します。時系列で隣接する互換性のあるツール呼び出しはグループ化され、詳細は必要なときだけ読み込まれます。ユーザー向けに表示可能な thinking は既定で折りたたまれ、artifact、承認、ユーザー操作が必要なタスク状態は独立した Timeline 項目として表示されます。回復済みの内部失敗と成功した検証はメインフローに表示せず、真の終端失敗は分かりやすい結果を最大 1 件だけ表示します。raw ツールエラー、出力、検証証拠は **実行詳細** に保持されます。実行グラフはメインフローに常駐せず、raw canonical event は開発者診断に限定されます。長い履歴は仮想化され、上へスクロールすると自動追従が解除されるため、ストリーミング更新によって表示位置が末尾へ強制的に戻されることはありません。Phase 5 では legacy/shadow レンダラーと runtime rollout flag を削除しました。ロールバックはコードまたはバージョンの復元で行い、transcript や保存済み会話の移行は不要です。
...
JunFeiAI 管理インストールでは、ログイン済み Relay アカウント用に `openai` を予約します。起動時に旧 `lingzhiwuxian/*` 参照と、以前の個人 OpenAI アカウントまたはエンドポイントを、管理対象のネイティブ Responses 設定へ移行します。JSON の破損、モデル参照キーの衝突、書き込み失敗時は移行を中止します。
Gateway 起動前に、同じ冪等ブートストラップが管理対象の 372K コンテキストと設定済み推論既定値を持つ `openai/smart-latest` へチャットを収束させ、最初の Agent ターン前に画像・動画 Provider を準備します。各テキストモデル呼び出しは OpenAI Responses から開始し、可視出力またはツールの副作用が発生する前に Provider が失敗した場合、その呼び出しだけを `shared/junfeiai-endpoints.json` のフォールバックへ切り替えます。既定の `deepseek/deepseek-v4-pro` は OpenAI Relay と同じ Base URL と API Key を使用しつつ Chat Completions（`/chat/completions`）を呼び出し、次の呼び出しは再び OpenAI から開始します。
Agents ページでは、簡単な役割名と責任、内蔵アバターを選ぶだけで、モデルが実用的な Agent プロファイルと最初のメッセージを生成し、作成後すぐにその Agent 専用チャットへ移動できます。

チャット、画像、動画の新規リクエストは同じ OpenClaw Agent loop を共有します。モードはその turn のモデル、サイズ、品質、時間、選択済み artifact に対する構造化されたメディア既定値だけを渡します。これは Agent がネイティブメディアツールを選択した後にだけツール引数へ適用され、モデルプロンプトへ連結されることはありません。管理対象の動画生成は `shared/junfeiai-endpoints.json` の向き別設定（`854x480`、`480x854`、`480x480`）から 480P を既定にし、720P は明示的に選択できます。モードは Renderer 側の intent planner や media queue には分岐しません。ネイティブタスクと同梱の Host Task Bridge は `session/run/tool-call/idempotency` と、実 capability から導出した受け入れ要件を永続化します。タスク終端、artifact、検証、承認、部分失敗は同じ会話へ直接戻り、Renderer の別の完了 gate や無条件のモデル wake を第二の判定者として使いません。旧 direct planner の POST endpoint は `410` を返し、新規リクエストはすべて Agent loop に入ります。
...
### 💾 高性能ポータブルモード
macOS では `pnpm package:mac:usb`、Windows では `pnpm package:win:usb` でインストール不要のポータブルビルドを生成できます。設定、ログイン状態、OpenClaw 設定、Agent、セッション、スキル、チャネル認証情報、更新パッケージは同梱の `UClawData/` に保存されるため、別の PC に差しても以前の記録を確認できます。Python、uv、一時ファイル、ブラウザキャッシュ、コンパイルキャッシュなど再生成できるランタイムデータはホスト PC 側の `UClawRuntime/` に置き、USB の低速な読み書きや容量増加を抑えます。更新時、portable helper は新しいバージョンが重要な Main プロセス初期化の完了を確認するまで以前のアプリケーションファイルを保持します。起動に失敗するか 90 秒以内に確認できない場合は、`UClawData/` を置換または削除せず、以前のバージョンを自動的に復元して再起動します。

...
### 💾 高性能ポータブルモード
macOS では `pnpm package:mac:usb`、Windows では `pnpm package:win:usb` でインストール不要のポータブルビルドを生成できます。設定、ログイン状態、OpenClaw 設定、Agent、セッション、スキル、チャネル認証情報、更新パッケージは同梱の `UClawData/` に保存されるため、別の PC に差しても以前の記録を確認できます。Python、uv、一時ファイル、ブラウザキャッシュ、コンパイルキャッシュなど再生成できるランタイムデータはホスト PC 側の `UClawRuntime/` に置き、USB の低速な読み書きや容量増加を抑えます。

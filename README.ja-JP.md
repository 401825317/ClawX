...
Windows USB ZIP のルートには `UClaw-SelfCheck.cmd` が同梱されています。Node.js、Python、Git を別途インストールせずにダブルクリックするだけで、同梱ランタイム、ディレクトリの書き込みと atomic rename、プラグイン依存関係、ローカルポート、zz-cn 接続、OpenClaw Doctor の状態を確認できます。秘匿情報を除いたサポートレポートは `UClawData/diagnostics/` に保存され、USB が書き込み不可の場合はホスト PC の一時ディレクトリへ退避します。

...
JunFeiAI 管理インストールでは、プロバイダー設定から旧 `lingzhiwuxian/*` チャット参照を `openai/*` へ明示的に移行し、OpenClaw のネイティブ Responses アダプターを使用できます。旧 provider は互換経路として保持され、`xhigh` 推論と現在の Agent/永続タスク実行系も維持されます。出力開始前に `/responses` が 404 を返した場合だけ Chat Completions に一度フォールバックします。

...
デフォルトの会話レンダラーは、OpenClaw と Gateway の事実をイベント駆動の線形 Timeline として投影します。時系列で隣接する互換性のあるツール呼び出しはグループ化され、詳細は必要なときだけ読み込まれます。ユーザー向けに表示可能な thinking は既定で折りたたまれ、artifact、検証結果、承認、サブタスクは独立した Timeline 項目として表示されます。実行グラフはメインフローに常駐せず、必要なときだけ **実行詳細** から開きます。raw canonical event は開発者診断に限定されます。長い履歴は仮想化され、上へスクロールすると自動追従が解除されるため、ストリーミング更新によって表示位置が末尾へ強制的に戻されることはありません。Phase 5 の同等性、E2E、製品互換性、性能ゲートがすべて完了するまでは、旧 projection を移行時のロールバック経路としてのみ残します。

運用側は `client.rollouts.chatTimelineMode` で `timeline`、`legacy`、`shadow` を選択でき、緊急ロールバック用の起動時環境変数 `CLAWX_CHAT_TIMELINE_MODE` が最優先されます。`shadow` は旧レンダラーを表示したまま canonical event をバックグラウンドで reduce し、履歴 checkpoint と authoritative な run 終端でのみ意味的な比較を行います。比較診断はセッション単位で容量を制限し、メッセージ本文、ツール結果、認証情報を保持しません。

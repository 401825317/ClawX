...
ローカル成果物の実行は、レビュー、修復、検証のループで進みます。現在のテキストモデルが意味的な計画を優先して作成し、主題、長さ、操作などの明示的な制約は型付き実行コントラクトに保持されます。実成果物の検証と正規の会話配信が成功した後にのみ完了となり、後続の依頼には直近の構造化された成果物と配信状態が渡されます。

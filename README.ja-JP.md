...
`VideoProject` は最終動画リクエストごとの永続プロジェクトで、単一クリップと複数ショット動画の両方を扱います。創作制約、参照画像の来歴、安定したショット ID、provider 試行、QA 判断、合成、配信を記録します。実際の生成は既存の `video_generate` を再利用し、UClaw Host は各ショットの決定可能なメディア事実を検証して Agent の意味的レビュー用 contact sheet を出力します。ローカル OCR や主観的な映像・音声評価を実装済みであるかのようには扱いません。
...
### 🔔 更新通知
ClawX は起動時に新しいバージョンを自動確認できます。更新が見つかるとアプリ内通知を表示し、ダウンロードやインストールはユーザーが選択した後にのみ実行されます。Windows のインストール版更新では、新しいファイルの展開が成功するまで以前のインストールディレクトリを保持し、展開に失敗した場合は以前のディレクトリを復元してから終了します。

### 💾 高性能ポータブルモード
macOS では `pnpm package:mac:usb`、Windows では `pnpm package:win:usb` でインストール不要のポータブルビルドを生成できます。設定、ログイン状態、Chromium セッション状態、OpenClaw 設定、Agent、セッション、スキル、チャネル認証情報は同梱の `UClawData/` に保存されるため、別の PC に差しても以前の記録を確認できます。更新ダウンロード、Python、uv、一時ファイル、ログ、クラッシュダンプ、ブラウザのディスクキャッシュ、コンパイルキャッシュはホスト PC 側の `UClawRuntime/` に置き、USB の低速な読み書きや容量増加を抑えます。新しい配布物の `UClawData/` は必ず空の状態に初期化され、ビルド PC のアカウントやランタイム状態を引き継ぎません。更新時、portable helper は現在のプロセスが終了したことを確認してからファイルを置換し、新しいバージョンが重要な Main プロセス初期化を完了するまで以前のアプリケーションファイルを保持します。旧バージョンの終了を確認できない場合は、ファイルを置換しません。パッケージ検証、展開、置換、起動、または 90 秒以内の起動確認に失敗した場合は、`UClawData/` を置換または削除せず、以前のバージョンを自動的に復元して再起動します。

...
> 管理対象の動画モデル、モード、サイズ、時間、およびモデルごとの既定値は、バックエンドの capability contract を唯一の基準とします。UClaw は一時的な障害に備えて最後の有効な contract を保存し、ユーザーが最大サイズや最長時間を明示しない限りモデルの既定値を使います。新規端末がオフラインで有効な contract を持たない場合、推測値を送らず管理対象動画を無効にします。
...
各セッションでは、チャットツールバーから個別のプロジェクトディレクトリを選択できます。ClawX はそのディレクトリをセッションとともに復元し、Agent とツールの実行に使用して、ツールが返したローカルファイル成果物を配下の `outputs/` に保存します。未指定の場合は Agent ワークスペースを使用し、そのセッションの実行中はディレクトリを変更できません。

ローカル成果物の実行は、レビュー、修復、検証のループで進みます。現在のテキストモデルが意味的な計画を作成し、主題、長さ、メディア、操作などの明示的な要件は、実際に選択された capability または永続タスクとともに保存されます。完了は実ツール/タスクの終端状態、capability 固有の検証、正規の会話配信から決まり、モデルがメタデータツールで自身の実行を承認または証明することはありません。
単独のプレゼンテーション依頼は `presentation-maker` ビジュアルスタジオを使います。エージェントがストーリー、素材、各スライドの構図を設計し、`create_designed_pptx_file` が PptxGenJS の自由キャンバスに画像、グラフ、表、図形、テキストを書き込みます。5 種類の内蔵テーマは `create_pptx_file` と移行前ジョブの復旧時の基本フォールバックに限定され、高品質デザインの主経路としては扱いません。
各 Agent は `provider/model` の実行時設定を個別に上書きできます。上書きしていない Agent は引き続きグローバルの既定モデルを継承します。
JunFeiAI 管理インストールでは、ログイン済み Relay アカウント用に `openai` を予約します。起動時に旧 `lingzhiwuxian/*` 参照と、以前の個人 OpenAI アカウントまたはエンドポイントを、管理対象のネイティブ Responses 設定へ移行します。旧 provider は Chat Completions の互換経路として残りますが、JSON の破損、モデル参照キーの衝突、書き込み失敗時は移行を中止します。
Gateway 起動前に、同じ冪等ブートストラップが管理対象の 372K コンテキストと設定済み推論既定値を持つ `openai/smart-latest` へチャットを収束させ、最初の Agent ターン前に画像・動画 Provider を準備します。ネイティブ Responses は、出力開始前に `/responses` が 404 を返した場合だけ Chat Completions に一度フォールバックします。
Agents ページでは、簡単な役割名と責任、内蔵アバターを選ぶだけで、モデルが実用的な Agent プロファイルと最初のメッセージを生成し、作成後すぐにその Agent 専用チャットへ移動できます。

チャット、画像、動画の新規リクエストは同じ OpenClaw Agent loop を共有します。モードはその turn の構造化されたメディア制約と選択済み artifact だけを渡します。Agent は現在の依頼、参照入力、公開済み provider capability からネイティブメディアツールを使うか判断し、動画モデルを明示的に選択します。動画モードのサイズと時間はツール選択後にだけ引数へ適用され、設定済み UI 既定モデルが provider override として黙って注入されることはありません。チャットと動画が同じ `openai` provider 名前空間を使っていても、チャットモデルが動画候補になることはありません。`video_generate` は provider リクエスト前に未公開モデルを拒否し、別モデルへ黙って置き換えません。これらの値がモデルプロンプトへ連結されることもなく、モードは Renderer 側の intent planner や media queue には分岐しません。ネイティブタスクと同梱の Host Task Bridge は `session/run/tool-call/idempotency` と、実 capability から導出した受け入れ要件を永続化します。タスク終端、artifact、検証、承認、部分失敗は同じ会話へ直接戻り、Renderer が別の完了判定を行うことはありません。`yield` 後のセッションが永続化された完了注入を消費する必要がある場合、安定したタグを持つ同一セッションの完了通知 turn は最大 1 回だけスケジュールされます。旧 direct planner の POST endpoint は `410` を返し、新規リクエストはすべて Agent loop に入ります。

`VideoProject` は最終動画リクエストごとの永続プロジェクトで、単一クリップと複数ショット動画の両方を扱います。創作制約、参照画像の来歴、安定したショット ID、provider 試行、QA 判断、字幕、ナレーション/音声方針、合成、配信を記録します。実際の生成は既存の `video_generate` を再利用し、UClaw Host は各ショットの決定可能なメディア事実を検証して Agent の意味的レビュー用 contact sheet を出力します。ローカル OCR や主観的な映像・音声評価を実装済みであるかのようには扱いません。実測したソース寸法は送信済み生成契約を満たす必要があり、低解像度出力を拡大して合格扱いにはできません。すべてのプロジェクトは Host の最終 QA を通り、そのワークフローだけが同一の検証済み成果物を assembled または delivered にできます。
ローカル動画合成では、動画モデルが生成した元の音声を既定で保持します。ニューラルまたはシステム TTS は、ユーザーが明示的にナレーションを求めた場合、または元動画に利用可能な音声がない場合だけ任意のレイヤーとして使い、すべての生成音声を無条件に置き換えません。
...
### 🧩 拡張可能なスキルシステム
事前構築されたスキルでAIエージェントを拡張できます。統合 Skills ページはローカル優先で、管理ディレクトリや workspace のスキルをスキャンし、Gateway に依存せず有効/無効を切り替えられます。OpenClaw runtime が利用できる場合は、公開 ClawHub marketplace からコミュニティスキルを検索・インストールできます。
Skills ページでは OpenClaw の複数ソース（管理ディレクトリ、workspace、追加スキルディレクトリ）から検出されたスキルを表示でき、各スキルの実際のパスを確認して実フォルダを直接開けます。OpenClaw 同梱の bundled skill については、コミュニティ版ではパッケージにも表示にも `skill-creator` のみを残し、より広いカタログは公開 ClawHub 検索/インストールから利用できます。dev 起動時と packaged 起動時の両方で他の bundled skill を物理的に削除し、削除済み bundled skill の古い `openclaw.json` エントリも一緒に掃除します。
...
指定した画像形式、背景、互換性のある圧縮オプションは画像 Provider に転送されます。ユーザーが独自設定をしていない場合、管理対象インストールの生成メディア配信上限は 16 MiB になります。生成画像がその上限を超える場合、UClaw は Provider が正常に生成した結果を破棄せず、保存前に自動変換と段階的な圧縮を行います。内部完了メッセージが非表示でも、タスク台帳の終端状態によって待機 UI と計時は終了します。
...
置き換えた API キーの検証に成功すると、UClaw は Gateway を更新する前にそのアカウントの永続化された認証失敗状態を消去します。通常の 401 保護を維持したまま、修正済み Provider をすぐ再選択できます。
...
### 💾 高性能ポータブルモード
macOS では `pnpm package:mac:usb`、Windows では `pnpm package:win:usb` でインストール不要のポータブルビルドを生成できます。設定、ログイン状態、Chromium セッション状態、OpenClaw 設定、Agent、セッション、スキル、チャネル認証情報は同梱の `UClawData/` に保存されるため、別の PC に差しても以前の記録を確認できます。更新ダウンロード、Python、uv、一時ファイル、ログ、クラッシュダンプ、ブラウザのディスクキャッシュ、コンパイルキャッシュはホスト PC 側の `UClawRuntime/` に置き、USB の低速な読み書きや容量増加を抑えます。高頻度の OpenClaw 状態はポータブル ID ごとの `UClawRuntime/profiles/<portable-id>/openclaw-state` に隔離し、`UClawData/` にはポータブル ID、ユーザーデータ、検証済み Runtime スナップショットだけを残します。新しい配布物の `UClawData/` は必ず空の状態に初期化され、ビルド PC のアカウントやランタイム状態を引き継ぎません。

ポータブル更新では旧アプリの完全終了を確認してからトランザクション的にファイルを置き換えます。置換、ロールバック、または新しいウィンドウの起動確認に失敗した場合は、部分更新を成功として扱わず、復旧用バックアップとログを保持します。macOS のポータブルパッケージには Apple Silicon と Intel Mac の両方に対応する更新ヘルパーを含めます。

Windows のパッケージ処理は、コミット済みで clean な worktree を必須とし、古い unpacked ディレクトリと過去の USB 成果物を先に削除します。最終処理では、ソースのバージョンと Git commit を `app.asar` と照合し、同梱する 4 個の Windows 実行ファイルを x64 PE として検証し、12 個の UClaw 組み込みおよびチャネル/検索プラグインとそのランタイム依存関係を確認して、`uclaw-usb-build.json` とリリース用 JSON を生成します。識別情報または内容が一致しない場合、古い成果物を公開せずビルドを失敗させます。

Windows USB ZIP のルートには `UClaw-SelfCheck.cmd` が同梱されています。Node.js、Python、Git を別途インストールせずにダブルクリックするだけで、ビルド識別情報、同梱済みとインストール済みのプラグインコピー、同梱ランタイム、ディレクトリの書き込みと atomic rename、ローカルポート、zz-cn 接続、OpenClaw Doctor の状態を確認できます。動的チェックでは、直近 24 時間かつ現在のビルドより新しいログだけを走査します。秘匿情報を除いたサポートレポートは `UClawData/diagnostics/` に保存され、USB が書き込み不可の場合はホスト PC の一時ディレクトリへ退避します。
...
- コアランタイムの待受は常に**単一所有者**です。Host API `127.0.0.1:13210` と OpenClaw Gateway `127.0.0.1:18789` は、同じ UClaw デスクトップインスタンスに属している必要があります。
- インストール版とポータブル版が混在する更新時、UClaw はウィンドウを開く前に共有インスタンスロックと両ポートのプロセスツリーを確認します。旧 UClaw/ClawX と確認できた場合だけ終了オプションを表示し、未確認のプロセスを自動終了することはありません。
- 一時 OAuth コールバック、動的ループバックポート、開発サーバー、外部プロバイダーポートは、この起動ガードの接管対象ではありません。
...
- Listen プロセスの確認例:
  - macOS/Linux: `lsof -nP -iTCP:13210 -sTCP:LISTEN` と `lsof -nP -iTCP:18789 -sTCP:LISTEN`
...
pnpm run test:packaged:win       # 最新 Windows USB ZIP のコア実機回帰を実行
pnpm run test:packaged:win:full  # パッケージ版のチャット、ツール、ブラウザ、Cron、Agent、メディア回帰を実行
pnpm run comms:replay     # 通信リプレイ指標を算出
pnpm run comms:baseline   # 通信ベースラインを更新
pnpm run comms:compare    # リプレイ指標をベースライン閾値と比較

# ユニットテスト
# このプロジェクトではユニットテストを維持しません。製品挙動はプロジェクトオーナーがローカルで手動検証します。
...
pnpm package:mac:usb      # macOS向けインストール不要の高性能ポータブルフォルダを生成
pnpm package:win           # Windows向けにパッケージ化
pnpm run package:win:verify # Windows NSIS インストーラーのライフサイクルを検証
pnpm package:win:usb       # Windows USB ZIP を生成し、パッケージ版の完全回帰を自動実行
pnpm run release:build      # ビルドと検証のみ（公開しない）
pnpm run release            # ビルド、検証後に最終成果物を公開

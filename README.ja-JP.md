...
管理対象 UClaw ディストリビューションは、Electron Main の Host API を通じて、設定済みの本番バックエンド（`https://aiwxxx.com`）が公開する有効な告知も読み込みます。この読み取りは認証不要で、アカウント資格情報を送信しません。告知は公開日時と有効期限で絞り込み、通常・重要・緊急の 3 レベルを扱います。重要な告知は各項目につき一度だけアプリ内トーストを表示し、緊急の告知は閉じるまでブロッキングダイアログで確認を求め、サイドバーのベルには現在の未読一覧を保持します。外部リンクは有効な HTTP(S) URL の場合だけ開きます。一時的な取得失敗では最後の有効なフィードを保持し、無効または不正な設定は安全側で無効化します。コミュニティ版は管理対象の告知を取得しません。

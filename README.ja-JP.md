...
### テキスト実行の効率
準備処理では `pi` を内蔵ランタイムの別名として認識し、外部 CLI の探索を省きます。MCP/LSP 設定は現在のプラグイン索引から bundle のマニフェストだけを読み込みます。プラグインの有効化、設定更新、認証情報の解決、実行ごとのツール状態は従来どおり扱います。

Tool Search の既定値は `directory` です。すべてのツール定義を毎回送信せず、検索・説明・実行を通じてツールを利用できます。明示的な設定は保持されます。OpenClaw 2026.6.10 は不変の内蔵モデルカタログを再利用しますが、認証情報はキャッシュせず、`smart-latest` などの動的エイリアスを特定モデルに固定しません。Provider の生成時間とローカル準備時間は別々に計測します。
...
Sentry へ送信する前に、クライアントは認識済みの資格情報、Cookie、prompt、ファイル内容、リクエスト本文、レスポンス本文のフィールドを削除し、Bearer/クエリ資格情報をフィルターします。ローカルファイルシステムのパスは、失敗した実行場所を特定できるよう診断情報に保持されます。これは多層防御であり、任意内容に対する絶対的な隔離ではありません。未知のフィールド内にある機密テキストが残る可能性があり、サーバー tunnel は受け入れた Sentry Envelope を二次的な内容スクラブなしで転送します。
...
macOS では、パッケージ済みの arm64/x64 クライアントはすべて、`latest-mac.yml` ではなく管理対象の `package_type=portable_zip` フィードを確認します。更新パッケージの種類とデータモードは別に判定され、同じ起動ルートに `portable.flag`、`UClawData`、`UClaw.app` がそろい、関連するディレクトリが書き込み可能な場合だけ自動置換を行います。DMG から起動したアプリ、単独で `/Applications` に移動したアプリは ZIP をダウンロードできますが、新しい書き込み可能なフォルダーへ ZIP 全体を展開して手動で移行してください。インストール済み App の横に `UClawData` を作成することはありません。2.0.3 は従来の分岐を使うため、最初のアップグレードは手動で完全な新 ZIP をインストールする必要があります。macOS のポータブル自動更新は修正版 2.0.4（またはそれ以降）から有効です。
...
パッケージ済みの USB ポータブル版では、初回起動時に実行環境の準備画面が表示されることがあります。変更しない OpenClaw ランタイムを本機のキャッシュへコピーし、検証、クリーンアップ、スキャン、ファイル数/バイト数のコピー進行状況、最終有効化の状態を表示します。完了するまで USB ドライブを接続してください。次回以降は完全なローカルキャッシュを再利用するため、通常はコピーをスキップします。
...
Windows USB ビルドは `UClaw-<version>-win-x64-usb.zip` と対応する JSON メタデータを生成します。永続データは `UClawData` に保存し、頻繁に更新される状態は分離されたローカル実行プロファイルを使用します。ポータブル更新は ZIP、サイズ、SHA-512 の検証後にのみインストールされます。置換中は再帰的なファイル数とバイト数の進行状況を表示し、Windows の同一ボリュームでは検証済みファイルを直接移動し、別ボリュームへのコピーは最大 4 ワーカーに制限します。`UClawData` は置換とロールバックの対象外です。パッケージには外部の `resources/bin/UClawRepair.exe` 修復ヘルパーも含まれ、匿名化された診断レポートを生成し、確認後または `--repair` 指定時に限り、更新一時ディレクトリの隔離、明確に所有された UClaw/OpenClaw プロセスの終了、再起動試行だけを行います。`UClawData`、SQLite、セッション、設定、資格情報は削除しません。`pnpm release` は Windows 専用のローカル候補ビルドです。クリーンな Git ワークスペースを要求し、USB パッケージを作成して正確なバージョン、コミット、build ID、サイズ、SHA-512 を検証しますが、機能回帰は実行しません。署名、アップロード、本番更新フィードの変更、Git タグ、GitHub Release の作成は行いません。`.github/workflows/uclaw-portable-production.yml` は既定では GitHub ホステッド Runner で不変の未署名 Windows/macOS USB 候補を作成します。明示承認された保護ステージングでは、SignPath で Windows の UClaw 実行ファイル、修復ヘルパー、更新ヘルパーに署名し、macOS App を署名・公証し、最終バイトから ZIP メタデータを再生成してから、OSS と `enabled=false` の zz-cn レコードを書き込みます。

...
チャット、画像、動画の新規リクエストは同じ OpenClaw Agent loop を共有します。モードはその turn のモデル、サイズ、品質、時間、選択済み artifact に対する構造化されたメディア既定値だけを渡します。これは Agent がネイティブメディアツールを選択した後にだけツール引数へ適用され、モデルプロンプトへ連結されることはありません。モードは Renderer 側の intent planner や media queue には分岐しません。ネイティブタスクと同梱の Host Task Bridge は `session/run/tool-call/idempotency` と、実 capability から導出した受け入れ要件を永続化します。タスク終端、artifact、検証、承認、部分失敗は同じ会話へ直接戻り、Renderer の別の完了 gate や無条件のモデル wake を第二の判定者として使いません。旧 direct planner の POST endpoint は `410` を返し、新規リクエストはすべて Agent loop に入ります。

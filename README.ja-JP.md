...
各セッションでは、チャットツールバーから個別のプロジェクトディレクトリを選択できます。ClawX はそのディレクトリをセッションとともに復元し、Agent とツールの実行に使用して、複合タスクのローカルファイル成果物を配下の `outputs/` に保存します。未指定の場合は Agent ワークスペースを使用し、そのセッションの実行中はディレクトリを変更できません。
...
単独のプレゼンテーション依頼は `presentation-maker` ビジュアルスタジオを使います。エージェントがストーリー、素材、各スライドの構図を設計し、`create_designed_pptx_file` が PptxGenJS の自由キャンバスに画像、グラフ、表、図形、テキストを書き込みます。5 種類の内蔵テーマは `create_pptx_file` と複合ランタイムの基本フォールバックに限定され、高品質デザインの主経路としては扱いません。

...
test.describe('Cron job card layout', () => {
  ...
  test('makes a completed-but-undelivered cron run and its consumption attribution explicit', async ({ electronApp, page }) => {
    await installIpcMocks(electronApp, {
      ...
      hostApi: {
        ...
        [stableStringify(['/api/cron/jobs', 'GET'])]: {
          ok: true,
          data: { status: 200, ok: true, json: cronJobs },
        },
        [stableStringify(['/api/channels/accounts', 'GET'])]: {
          ok: true,
  ...
  test('shows and clears an explicit legacy cron timeout', async ({ electronApp, page }) => {
    await installIpcMocks(electronApp, {
      gatewayStatus: { state: 'running', port: 18789, pid: 12345, gatewayReady: true },
      gatewayRpc: {},
      hostApi: {
        ...
        [stableStringify(['/api/cron/jobs/job-delivery-failure/clear-timeout', 'POST'])]: {
          ok: true,
    ...
    await expect(page.getByText('执行时限 5 分钟')).toBeVisible();
    await page.getByRole('button', { name: '恢复自动时限' }).click();
    await expect(page.getByText('已恢复自动时限')).toBeVisible();
  });
});

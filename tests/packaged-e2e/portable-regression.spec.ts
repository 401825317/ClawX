...
async function waitForGeneratedMedia(
  ...
  const artifact = kind === 'image'
    ? page.locator('[data-testid="acp-image-part"]:visible').first()
    : page.locator('[data-testid="acp-video-player"]:visible').first();
  const indicator = page.getByTestId(kind === 'image'
    ? 'chat-composer-image-generation-indicator'
    : 'chat-composer-video-generation-indicator');
...
async function runLiveRegression(runner: ScenarioRunner): Promise<void> {
  ...
  await runner.run('live.managed.contract', 'verify managed Responses and media runtime configuration without reading secret values', async () => {
    ...
    expect(allow).toEqual(expect.arrayContaining(['clawx-openai-image', 'openai', 'uclaw-video']));
    return {
  ...
  await runner.run('live.media.image', 'generate and render one real managed image', async () => {
    ...
    await expect(page.getByTestId('chat-composer-image-generation-indicator')).toHaveCount(0, { timeout: 60_000 });
    return { latencyMs };
  });
  ...
  await runner.run('live.media.video', 'generate and render one real managed video', async () => {
    ...
    await expect(page.getByTestId('chat-composer-video-generation-indicator')).toHaveCount(0, { timeout: 60_000 });
    return { latencyMs };
  });
  ...
  try {
    ...
    await runner.run(
      ...
      async () => {
        ...
        await startNewChat(page);
        return { attempts, composerRecovered: true };
      },
    ...
    const localArtifactsPluginPath = path.join(
      appRoot,
      'resources',
      'openclaw-plugins',
    ...
    const hasLocalArtifactsPlugin = await access(localArtifactsPluginPath).then(() => true).catch(() => false);

...
| `full` | 是 | `core` 加本地确定性 Provider、真实 OpenClaw 文本流、Markdown/Unicode/多轮、429/500 自动重试、畸形流、401 凭证恢复、取消、文件工具、浏览器工具、Agent、Cron、FFmpeg 时间线/合成/Shot QA 及错误输入 |
| `live` | 显式执行 | 使用专用测试账号执行托管 Responses、真实图片和真实视频；外部渠道发送仍需额外授权 |
...
| Provider | 本地兼容 Provider 校验、保存、设为默认、重启后保留 | 无效 API Key 必须被拒绝；401 后重新校验并替换凭证会清除该账号的持久化认证失败状态 | `full` |
| 文本聊天 | 简单、中文、多语言、Markdown、表格、代码、多轮上下文 | 瞬时 429/500 自动重试、畸形流失败、慢请求在到达 Provider 后取消、401 凭证修复后恢复 | `full` |
| 工具 | OpenClaw 写文件、浏览器打开与 snapshot | 工具缺失、浏览器不可用或没有真实副作用均失败 | `full` |

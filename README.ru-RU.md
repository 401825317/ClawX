...
Расширяйте возможности AI-агентов готовыми навыками. Локальная страница Skills сканирует управляемые каталоги и workspace и позволяет включать или отключать навыки без зависимости от Gateway.
UClaw не устанавливает отдельные копии `pdf`, `xlsx`, `docx` и `pptx`; старые копии удаляются только при наличии маркера владения UClaw. Полный набор встроенных навыков OpenClaw сохраняется как в режиме разработки, так и в упакованной сборке.
Дополнительно UClaw предоставляет совместимые навыки `presentation-maker`, `spreadsheet-maker`, `document-maker` и `blender-maker`. Модель выбирает их в обычном цикле OpenClaw Agent: Office-файлы создаёт локальная среда артефактов без hook-ов prompt или transcript, а Blender получает только проверенный декларативный SceneSpec через приватный loopback-мост. Сам Blender не входит в пакет; если он не установлен, инструмент явно сообщает о недоступности возможности и не выдаёт незавершённую задачу за готовую.
...
В упакованной USB-версии при первом запуске может появиться экран подготовки среды выполнения: неизменяемая среда OpenClaw копируется в локальный кэш компьютера, а экран показывает проверку, очистку, сканирование, прогресс по файлам и байтам и финальную активацию. Не отключайте USB-накопитель до завершения. При следующих запусках полный локальный кэш переиспользуется, поэтому копирование обычно пропускается.
...
pnpm package:win:usb          # Собрать Windows x64 USB ZIP (только Windows, требуется чистое дерево)
pnpm package:win:verify       # Собрать и выполнить финальную проверку NSIS и метаданных
pnpm package:linux            # Упаковать для Linux
```

Windows USB-сборка создаёт `UClaw-<version>-win-x64-usb.zip` и соответствующий JSON-файл метаданных. Постоянные данные хранятся в `UClawData`, часто изменяемое состояние использует изолированный локальный профиль, а переносимое обновление устанавливается только после проверки ZIP, размера и SHA-512. После подписи Windows release-процесс пересоздаёт blockmap и метаданные NSIS, затем проверяет установку, запуск, обновление поверх существующей версии, удаление и сохранение пользовательских данных.

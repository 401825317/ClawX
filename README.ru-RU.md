...
Рендерер разговоров проецирует факты OpenClaw и Gateway в единую линейную Timeline, управляемую событиями. Совместимые вызовы инструментов, расположенные рядом по времени, объединяются, а их подробности загружаются по запросу; доступные пользователю рассуждения по умолчанию свёрнуты; артефакты, подтверждения и требующие действия состояния задач остаются самостоятельными элементами Timeline. Восстановленные внутренние ошибки и успешные проверки не попадают в основной поток; настоящий конечный сбой показывает не более одного понятного результата, а исходные ошибки инструментов, вывод и доказательства проверки остаются в **Подробностях выполнения**. Граф выполнения больше не занимает основной поток, а необработанные канонические события доступны только в диагностике разработчика. Длинная история виртуализирована; прокрутка вверх отключает автоматическое следование, поэтому потоковые обновления не возвращают экран принудительно вниз. Phase 5 удалил legacy/shadow-рендереры и runtime rollout flags; откат выполняется возвратом кода или версии и не требует миграции transcript либо сохранённых разговоров.
...
Управляемая установка JunFeiAI резервирует `openai` для учётной записи Relay, с которой выполнен вход. До запуска Gateway она переводит старые ссылки чата `lingzhiwuxian/*`, а также прежние личные учётные записи или endpoint OpenAI в управляемую нативную конфигурацию Responses с контекстом 372K и настроенным уровнем рассуждений. Тот же идемпотентный bootstrap заранее настраивает provider и модели для изображений и видео. Генерация видео по умолчанию использует 480P (`854x480`, `480x854` или `480x480` в зависимости от ориентации) из `shared/junfeiai-endpoints.json`; 720P остаётся доступным при явном выборе. Повреждённый JSON, конфликт ключей ссылок на модели и ошибки записи по-прежнему останавливают миграцию. Каждый вызов текстовой модели сначала использует OpenAI Responses; если Provider завершается ошибкой до видимого вывода или побочного эффекта инструмента, только текущий вызов переключается на резервный маршрут из `shared/junfeiai-endpoints.json`. По умолчанию `deepseek/deepseek-v4-pro` использует тот же Base URL и API key Relay, но вызывает Chat Completions (`/chat/completions`), а следующий вызов снова начинается с OpenAI.

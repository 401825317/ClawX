...
На macOS команда `pnpm package:mac:usb`, а на Windows команда `pnpm package:win:usb` создаёт портативную сборку, не требующую установки. Настройки, состояние входа, конфигурация OpenClaw, агенты, сеансы, навыки и учётные данные каналов хранятся в каталоге `UClawData/` рядом с приложением, поэтому данные можно переносить вместе с USB-накопителем. Восстанавливаемые данные среды выполнения, включая обновления, Python, uv, временные файлы, журналы и кэши, хранятся на компьютере в `UClawRuntime/`. Перед заменой файлов helper подтверждает завершение текущего процесса и сохраняет файлы предыдущей версии, пока новая версия не подтвердит критическую инициализацию Main-процесса. Если завершение прежнего процесса подтвердить нельзя, файлы не заменяются. При ошибке проверки, распаковки, замены, запуска или 90-секундного подтверждения helper автоматически восстанавливает и перезапускает предыдущую версию, не заменяя и не удаляя `UClawData/`. Установщик Windows также сохраняет предыдущий каталог установки до успешной распаковки и восстанавливает его при ошибке распаковки.

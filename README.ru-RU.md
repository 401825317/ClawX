...
В управляемой установке JunFeiAI можно явно перенести старые ссылки чата `lingzhiwuxian/*` в `openai/*` и использовать нативный адаптер Responses в OpenClaw. Старый provider сохраняется для совместимости, а `xhigh` и текущая среда Agent/долгих задач остаются без изменений. Откат к Chat Completions выполняется один раз только при ответе 404 от `/responses` до начала вывода.

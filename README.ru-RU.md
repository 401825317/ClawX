...
Рендерер разговоров проецирует факты OpenClaw и Gateway в единую линейную Timeline, управляемую событиями. Совместимые вызовы инструментов, расположенные рядом по времени, объединяются, а их подробности загружаются по запросу; доступные пользователю рассуждения по умолчанию свёрнуты; артефакты, результаты проверки, подтверждения и подзадачи остаются самостоятельными элементами Timeline. Граф выполнения больше не занимает основной поток: при необходимости откройте **Подробности выполнения**, а необработанные канонические события доступны только в диагностике разработчика. Длинная история виртуализирована; прокрутка вверх отключает автоматическое следование, поэтому потоковые обновления не возвращают экран принудительно вниз. Phase 5 удалил legacy/shadow-рендереры и runtime rollout flags; откат выполняется возвратом кода или версии и не требует миграции transcript либо сохранённых разговоров.

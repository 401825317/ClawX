...
Управляемая установка JunFeiAI теперь автоматически переводит старые ссылки чата `lingzhiwuxian/*` в `openai/*` до запуска Gateway и использует нативный адаптер Responses OpenClaw с управляемым контекстом 372K и настроенным уровнем рассуждений по умолчанию. Тот же идемпотентный bootstrap заранее настраивает provider и модели для изображений и видео. Личный endpoint OpenAI никогда не перезаписывается: при конфликте сохраняются старый путь Chat Completions и явное действие миграции. Откат к Chat Completions выполняется один раз только при ответе 404 от `/responses` до начала вывода.

...
The default conversation renderer now projects OpenClaw and Gateway facts into an event-driven, linear Timeline. Chronologically adjacent compatible tool calls are grouped with details loaded on demand, user-displayable thinking is collapsed by default, and artifacts, verification results, approvals, and subtasks remain independent timeline items. The execution graph no longer occupies the main flow: open **Execution details** only when needed, while raw canonical events remain limited to developer diagnostics. Long histories are virtualized, and scrolling upward detaches auto-follow so streaming updates do not pull the viewport back to the bottom. The legacy projection remains only as a migration rollback path until the Phase 5 parity, E2E, product-compatibility, and performance gates are complete.

Operators can select `timeline`, `legacy`, or `shadow` through `client.rollouts.chatTimelineMode`; the startup override `CLAWX_CHAT_TIMELINE_MODE` takes precedence for emergency rollback. `shadow` keeps the legacy renderer visible while reducing canonical events in the background and compares only history checkpoints and authoritative run terminals. Comparison diagnostics are session-bounded summaries and never retain message text, tool results, or credentials.

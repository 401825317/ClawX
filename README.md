...
Top-level address parsing recognizes HTTP, HTTPS, and explicit standard `file:///` syntax, but privileged navigation rejects direct `file:` URLs. Plain filesystem paths and other protocols are rejected. Workspace HTML uses a short-lived Main-owned tokenized loopback preview; expiry, revocation, feature-gate shutdown, and replacement fail closed. Private-network and DNS-unverified destinations are blocked. Allowed popup targets replace the current page rather than creating a child window; this same-page fallback cannot preserve `window.opener`, returned window handles, initially blank scripted popups, or full POST-body, referrer, named-window, and window-feature behavior.
...
When a newly completed, live tool call returns a structured successful webpage artifact with an absolute `.html` or `.htm` path, Chat automatically opens the last qualifying result in the built-in browser. Historical ACP replay never triggers this behavior. Renderer performs an initial workspace-boundary check, then Electron Main resolves the real workspace and file paths, verifies that the target is an existing file inside that workspace, and serves it through a tokenized `http://127.0.0.1` preview with bounded lifetime, instance count, and memory. The preview blocks external navigation, downloads, popups, clipboard, camera, and microphone access. A later successful result for the same path refreshes the preview. The independent `features.htmlPreview` gate is safe-off by default.
...
UClaw also provides `presentation-maker`, `spreadsheet-maker`, `document-maker`, and `blender-maker` compatibility skills. The model selects them inside the normal OpenClaw Agent loop: office outputs are written by the local artifact runtime without prompt or transcript hooks, while Blender receives only a validated declarative SceneSpec through a private loopback bridge. Blender itself is not bundled; when it is missing, the tool reports that capability as unavailable instead of claiming completion.

### Text Runtime Efficiency
Preparation also recognizes `pi` as the built-in runtime alias without external CLI discovery. Bundle MCP/LSP configuration reads only bundle plugin manifests selected by the current plugin index. Plugin activation, configuration refresh, credential resolution, and per-run tool state are not cached or bypassed by these changes.

The default Tool Search mode is `directory`: tools remain available through search, description, and execution without resending every full tool schema on each prompt. Explicit Tool Search settings are preserved. OpenClaw 2026.6.10 also reuses immutable bundled model catalogs; this does not cache credentials or pin a dynamic model alias such as `smart-latest` to an upstream model. Provider generation time and local preparation time must be measured separately.

### Deterministic Artifact Tasks
For managed UClaw deployments, Electron Main classifies explicit create or modify requests for presentations, documents, spreadsheets, webpages, CAD drawings, and ecommerce main images before dispatch. A clean chat remains in place; unrelated existing history creates a separate visible artifact session. A modification reuses a prior artifact session only for the same Agent, workspace, and known target file. The server-delivered `features.artifacts` gate is applied through a stable installation-ID bucket and rechecked by the relay. Its versioned model alias, such as `uclaw-artifact-v1`, remains available to the runtime but is hidden from the normal model picker with `visible: false`.

Fast tasks use `thinking=minimal`, local artifact tools and explicit attachments, with no network, shell, or decorative image generation and at most one targeted repair; ecommerce main images may still use image generation because it is their output tool. Explicit refinement or asset-search intent selects refined mode with `thinking=high`, conditional network or generated assets, and at most two repairs. The task log records the classifier result, prompt-contract version, model alias, thinking level, tool count, phase timings, and repair count, but does not log the user prompt body.

CAD requests use the versioned `cad-editor` contract and the local `create_dxf_file` tool. Delivery is a structurally validated text DXF inside the canonical workspace; PNG or arbitrary binary files cannot satisfy the CAD result. The versioned `ecommerce-main-image` skill distinguishes white-background, scene, and selling-point images. When a reference image is present, it must preserve product structure, packaging, logos, existing text, and other product facts.

Managed image and video model catalogs, supported sizes, qualities, durations, modes, and defaults come from the remote client configuration. The Chat composer exposes only the active model's declared values and does not invent a `480P`, square-image, quality, duration, or model fallback. If no valid remote capability is available, the corresponding generation mode is disabled. The media plugins validate selected values against the current runtime contract, preserve deliverable candidates from partially malformed image responses, and avoid blindly resubmitting terminal reference-image upload failures. Ecommerce rollout has its own safe-off gate and also requires the artifact gate.

### Long-Term Rules
Messages with an explicit permanence intent such as “always remember” or “make this permanent” can be saved as local long-term rules. An explicit global/all-agents request creates a global rule; otherwise it applies to the current Agent. Settings provides list, edit, enable/disable, delete, and undo operations, with version and update metadata.

Rules are stored under the active OpenClaw configuration directory and atomically projected into a dedicated managed block in each registered workspace's `AGENTS.md`; content outside that block is preserved. Main enforces Agent ownership for Agent-scoped rules. The independent `features.longTermRules` gate is safe-off by default; while disabled, rule APIs do not read or write the rule store or `AGENTS.md`. Rule text is never added to product-analytics or observability events, although an enabled projected rule necessarily becomes instruction context for the configured AI model.
...
UClaw managed distributions add account sign-in, registration, verification codes, and device authorization to first-run Setup. Credentials remain in Electron Main and the protected secret store. After sign-in or account switching, UClaw restarts Gateway automatically with the committed credentials; startup remains in progress until Gateway readiness is confirmed. After sign-in and on every startup, UClaw reconciles the canonical `openai` provider and the compatibility `lingzhiwuxian` provider so both use the same relay key and the same text-model set delivered by the new-api client configuration; the default route is `openai/<server default model>`. The Chat model picker displays only that server-delivered list. Existing third-party and image/video provider settings remain stored locally but are hidden from the managed Provider and model UI. Community distributions keep the existing Provider setup flow.
...
When UClaw publishes an enabled support contact, the sidebar also exposes Help & Support with one or more official QR contacts, work hours, notes, and optional WeChat ID copying. This public configuration does not require sign-in and does not affect Chat or Gateway state.
...
When Image Mode is enabled in Chat, the composer sends the remotely validated one-turn image model, size, and quality preferences through ACP. The agent model remains responsible for deciding whether to invoke `image_generate`; the composer never sends a direct image request.
...
### Diagnostics, Privacy, and Crash Safety
The local telemetry setting is the master consent switch. Turning it off shuts down PostHog, Main and Renderer Sentry clients, Crashpad uploads, and performance tracing; managed observability must also be enabled by the UClaw service before Sentry starts. Session Replay, screenshots, and default PII collection are disabled. Managed feature gates are safe-off when missing or invalid. High-risk runtime configuration uses a 15-second cache; after expiry, a refresh failure returns the affected capabilities to off instead of reusing an enabled value. The server tunnel remains the final observability kill switch.

Before Sentry submission, the client projects events onto a controlled diagnostic schema instead of sending prompts, file contents, request or response bodies, cookies, authentication headers, or credential values. Local filesystem paths are retained in diagnostics so support can identify the failing runtime location. The zz-cn tunnel decompresses and parses accepted envelopes, applies a second allowlisted projection, rejects malformed JSON and invalid lengths, discards attachments, minidumps, replay data and unverified binary payloads, and enforces compressed and decompressed size limits. Redis-backed rate limiting is shared across instances; if configured Redis fails, the tunnel fails closed rather than weakening the global limit.

A broken stdout/stderr pipe permanently disables only the affected console sink while file logging continues. Log files rotate at 20 MB, retain at most seven days, and are pruned to a 100 MB directory budget. Fatal handling has a single-entry guard, writes only a fixed safe diagnostic schema without the normal logger or raw exception text, and invokes Gateway/Blender cleanup only once before exit.

Requests to the exact configured UClaw backend origin receive version, Git commit, build ID, platform, architecture, update channel, runtime mode, a per-request ID, and a SHA-256 installation identifier. These headers are not injected into unrelated origins; the raw installation ID is never sent in this diagnostic header set.
...
Help & Support uses a read-only typed `support` Host API. Electron Main fetches and normalizes the public client configuration without credentials; unavailable or disabled configuration stays out of the sidebar and never changes authentication, Provider, OpenClaw, Chat, ACP, or Gateway state.
...
Accepted generated images, videos, and explicit assistant `MEDIA:` attachments keep their original session and turn ownership while another conversation is visible. Returning builds ordinary messages and tool order from ACP replay, then reapplies only the bounded marked media projection, so delayed delivery does not require a manual session refresh and does not duplicate normal history.

ACP retries rate-limit and service-unavailable failures (`429`/`503`) with bounded backoff only before any tool starts or visible assistant content is produced. Once a side effect or visible result exists, the turn is never replayed. Context overflow uses one bounded structured recovery. If media generation succeeded but a later summary step failed, Main reauthorizes media for the exact session, generation, and user turn; authorized images, audio, and video remain visible while unauthorized paths and raw `MEDIA:` directives remain hidden.
...
- Gateway ownership is recorded atomically with PID creation identity, runtime root, startup nonce, and a token digest. UClaw never terminates an unknown PID; takeover succeeds only after the listener and ownership record are both verified.
- Startup uses one bounded retry budget and absolute WebSocket/ready deadlines. Authentication failures are not retried, and the Gateway is marked running only after `gateway.ready` or a successful core RPC.
- Requested restarts wait for active model or tool runs to drain, with a 90-second upper bound and an explicit forced-restart reason. Post-restart channel probing is read-only and cannot delete channel configuration or trigger another restart.
- Channel synchronization captures one trusted startup snapshot. Missing or malformed channel state fails closed for destructive cleanup, so transient reads cannot remove configured WeChat or plugin state.
- Bundled plugin maintenance is ownership-aware. UClaw replaces or removes an extension only when a managed marker, an integrity-bound install record, or an exact bundled-content match proves ownership; same-name user extensions and ambiguous states are preserved and reported.
...
The Windows USB build emits `UClaw-<version>-win-x64-usb.zip` and matching JSON metadata. It keeps durable data in `UClawData`, uses an isolated local runtime profile for high-frequency state, and installs portable updates only after ZIP, size, and SHA-512 verification. Runtime snapshots use read-only SQLite integrity checks and online backup; an unstable source never publishes a partial snapshot or mutates the live database. A prior complete snapshot is retained when available, while first-time deferral is reported explicitly and retried with bounded exponential backoff outside Gateway startup. During replacement, the updater reports recursive file and byte progress, moves verified entries directly on the same Windows volume, and bounds cross-volume copying to four workers; `UClawData` remains outside replacement and rollback. The package also includes the external `resources/bin/UClawRepair.exe` helper; it can collect a redacted launch report and, after confirmation or `--repair`, only isolate update staging directories, stop clearly owned UClaw/OpenClaw processes, and try a restart without deleting user state. `pnpm release` is a Windows-only local candidate build: it requires a clean Git workspace, builds the USB package, and verifies the exact version, commit, build ID, size, and SHA-512 without running functional regression. It never signs, uploads, changes the production feed, creates a Git tag, or creates a GitHub Release. `.github/workflows/uclaw-portable-production.yml` builds immutable unsigned Windows and macOS USB candidates on GitHub-hosted runners. Its explicitly approved protected staging path signs the UClaw Windows executables with SignPath, signs and notarizes the macOS apps, rebuilds the ZIP metadata from those final bytes, and then uploads only disabled zz-cn release rows. Functional QA, live-account checks, and packaged-product regression remain separate workflows and are not implied by a successful build or publication.

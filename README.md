...
Fresh chat, image, and video turns now share the same OpenClaw Agent loop. Modes contribute only per-turn model, size, quality, duration, and selected-artifact preferences; they do not run a renderer-side intent planner or media queue. Native tasks and the bundled Host Task Bridge use Task Flow plus the same session/run/tool-call/idempotency contract for recoverable long-running work and same-session completion wake. UClaw projects structured progress, artifacts, verification, approvals, and partial failures into the execution UI instead of creating another Agent loop. Legacy composite read/cancel/retry operations remain only for recovering pre-migration jobs, while direct planner POST endpoints return `410`; new requests enter through the Agent loop.

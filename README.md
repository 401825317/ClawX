...
Managed JunFeiAI installs can explicitly migrate legacy `lingzhiwuxian/*` chat references to `openai/*` and use OpenClaw's native Responses adapter. The migration retains the legacy provider as a compatibility path, keeps `xhigh` reasoning and the current Agent/task runtime, and only falls back to Chat Completions when `/responses` returns 404 before output begins.

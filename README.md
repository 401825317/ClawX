...
When Image Mode is enabled in Chat, the composer sends one-turn image size and quality preferences (`medium` by default) through ACP. The agent model remains responsible for deciding whether to invoke `image_generate`; the composer never sends a direct image request.

...
The conversation renderer projects OpenClaw and Gateway facts into one event-driven, linear Timeline. Chronologically adjacent compatible tool calls are grouped with details loaded on demand, user-displayable thinking is collapsed by default, and artifacts, verification results, approvals, and subtasks remain independent timeline items. The execution graph no longer occupies the main flow: open **Execution details** only when needed, while raw canonical events remain limited to developer diagnostics. Long histories are virtualized, and scrolling upward detaches auto-follow so streaming updates do not pull the viewport back to the bottom. Phase 5 removed the legacy and shadow renderers and their runtime rollout flags; rollback is a code/version revert and requires no transcript or persisted-conversation migration.

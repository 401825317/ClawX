...
Managed UClaw distributions also read enabled public announcements from the configured production backend (`https://aiwxxx.com`) through the Electron Main Host API. This read is unauthenticated and never sends account credentials. Announcements are filtered by publication and expiry time and support normal, important, and urgent levels: each important notice shows a one-time in-app toast, urgent notices require dismissal in a blocking dialog, and the sidebar bell keeps the current unread list. External links are opened only when they are valid HTTP(S) URLs. A temporary fetch failure preserves the last valid feed; disabled or invalid configuration fails closed. Community distributions do not fetch managed announcements.

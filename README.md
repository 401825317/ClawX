...
Managed JunFeiAI installs automatically migrate legacy `lingzhiwuxian/*` chat references to `openai/*` during startup and use OpenClaw's native Responses adapter. Ambiguous personal OpenAI ownership is never overwritten and remains available for explicit handling in provider settings. The migration retains the legacy provider as a compatibility path, uses the configured reasoning default and the current Agent/task runtime, and only falls back to Chat Completions when `/responses` returns 404 before output begins.
Managed JunFeiAI installs now converge legacy `lingzhiwuxian/*` chat references to `openai/*` automatically before Gateway startup and use OpenClaw's native Responses adapter with the managed 372K context and configured reasoning default. The same idempotent bootstrap prepares the managed image and video providers before the first Agent turn. A personal OpenAI endpoint is never overwritten; that conflict preserves the legacy Chat Completions path and leaves the explicit migration action available. Native Responses only falls back to Chat Completions when `/responses` returns 404 before output begins.

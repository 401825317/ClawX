...
Desktop actions and Blender scenes use explicit runtime contracts rather than a normal chat completion: desktop control is session/run scoped, globally serialized, and requires an in-app approval token before an action can execute. The bundled Blender runtime accepts a declarative SceneSpec, runs local Blender without loading startup add-ons, verifies its `.blend`, `.glb`, preview, and manifest artifacts, and exposes the verified result in chat. Blender remains an optional local dependency; capability discovery reports unavailable platforms or executables instead of pretending a task completed. Chat previews load the 3D viewer only when a compatible model artifact is opened, keeping the normal chat path light.
Chat, image mode, and video mode now share the same OpenClaw Agent turn. Modes contribute only per-turn model, size, quality, duration, and selected-artifact preferences; they do not run a renderer-side intent planner or media queue. Native `image_generate` and `video_generate` tasks keep OpenClaw's task ledger and same-session completion wake, while UClaw projects progress, artifacts, verification, approvals, and partial failures into the execution UI. The bundled Host Task Bridge applies the same session/run/tool-call/idempotency contract to future recoverable local executors without creating another agent loop.

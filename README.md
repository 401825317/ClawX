...
The conversation renderer projects OpenClaw and Gateway facts into one event-driven, linear Timeline. Chronologically adjacent compatible tool calls are grouped with details loaded on demand, user-displayable thinking is collapsed by default, and artifacts, approvals, and actionable task states remain independent timeline items. Recovered internal failures and passed verification stay out of the main flow; a true terminal failure shows at most one friendly outcome, while raw tool errors, output, and verification evidence remain available in **Execution details**. The execution graph no longer occupies the main flow, and raw canonical events remain limited to developer diagnostics. Long histories are virtualized, and scrolling upward detaches auto-follow so streaming updates do not pull the viewport back to the bottom. Phase 5 removed the legacy and shadow renderers and their runtime rollout flags; rollback is a code/version revert and requires no transcript or persisted-conversation migration.
...
Fresh chat, image, and video turns share the same OpenClaw Agent loop. Modes contribute only per-turn structured media defaults for model, size, quality, duration, and selected artifacts; they are applied only after the Agent selects a native media tool and are never concatenated into the model prompt. Managed video generation defaults to 480P (`854x480`, `480x854`, or `480x480` by orientation) from `shared/junfeiai-endpoints.json`, while 720P remains explicitly selectable. Modes do not run a renderer-side intent planner or media queue. Native tasks and the bundled Host Task Bridge persist session/run/tool-call/idempotency identity together with capability-derived acceptance requirements. Terminal task, artifact, verification, approval, and partial-failure events return directly to the same conversation; the Renderer projects those facts without making another semantic completion decision or unconditionally waking the model for a completion announcement. Retired direct planner POST endpoints return `410`, and new requests enter through the Agent loop.
Each agent can also override its own `provider/model` runtime setting; agents without overrides continue inheriting the global default model.
Managed JunFeiAI installs reserve `openai` for the signed-in Relay account. During startup, legacy `lingzhiwuxian/*` references and any earlier personal `openai` account or endpoint are migrated to the managed native Responses configuration. Malformed JSON, model-reference collisions, and write failures still stop the migration.
Before Gateway starts, the same idempotent bootstrap converges chat on `openai/smart-latest` with the managed 372K context and configured reasoning default, and prepares managed image and video providers for the first Agent turn. Every text model call starts with OpenAI Responses; a Provider failure before visible output or tool side effects retries only the current call through the fallback configured in `shared/junfeiai-endpoints.json`. The default `deepseek/deepseek-v4-pro` route reuses the OpenAI Relay Base URL and API key but calls Chat Completions (`/chat/completions`), and the next call starts from OpenAI again.

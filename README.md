...
VideoProject is the durable envelope for every requested final video, whether it is a single clip or a multi-shot video: it records the creative constraints, reference-image lineage, stable shot identity, provider attempts, QA decisions, captions, narration/audio policy, composition, and delivery. The existing `video_generate` tool remains the generator. UClaw Host verifies deterministic media facts for each shot and emits a contact sheet for Agent semantic review; it does not claim local OCR or subjective visual/audio judgement. Measured source geometry must satisfy the submitted generation contract: undersized output is blocked and cannot be made acceptable by upscaling. Every project goes through Host final QA, and only that workflow can mark the exact verified artifact assembled or delivered.
...
Portable updates wait for the old app to exit before replacing files transactionally. If replacement, rollback, or the new window readiness check fails, the updater keeps the recovery backup and log instead of reporting a partial update as successful. macOS portable packages include updater helpers for both Apple Silicon and Intel Macs.

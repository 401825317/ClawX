...
- `src/components/client/`：托管客户端的全局轻量能力，目前包含重要公告铃铛、紧急公告弹层、帮助与客服多二维码入口，以及客户端配置初始化器。
...
- `GET /api/clawx/client-config`：读取 ClawX 客户端轻量配置，包括重要公告和帮助与客服多联系人二维码配置。renderer 通过本地 `/api/junfeiai/client-config` 代理读取，不直接请求远端后端。

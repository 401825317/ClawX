...
- **Global design before code changes**: Before making any ClawX/UClaw change, first consider the whole product/runtime path rather than only the local file being edited. For every proposed edit, explicitly account for its impact on the default chat model, image model, video model, chat/image/video modes, planner routing, execution queue, artifact delivery, verification gates, history refresh, UI progress display, packaging/dev-mode behavior, and existing user-restored behavior. Do not remove, rename, or reinterpret restored product affordances such as image mode/video mode without an explicit user decision. Prefer small implementation steps, but each step must be justified against the global design.
- **Codex-behavior reference principle**: For every user-facing UClaw/ClawX behavior issue, first ask how Codex would handle the same intent and failure mode, then map that answer back into this product's runtime contract. Prefer Codex-like semantics: clear current-turn intent classification, explicit side-effect authorization, structured run/artifact contracts, queue/backpressure for expensive tools, current-run evidence before completion, precise but friendly user-facing failure messages, and detailed diagnostics in logs. Do not fix incidents by hardcoding one user sentence or one historical accident when the durable fix belongs in planner contracts, state machines, tool guards, execution queues, artifact verification, or UI progress semantics.
- **No unit test policy**: This project no longer keeps or requires unit tests. Do not add new `tests/unit` specs, do not ask for unit-test coverage as a condition for a fix, and do not use `pnpm test` / Vitest unit tests as the default validation path. The user will do product-level manual verification locally. For code hygiene, use targeted static checks such as `pnpm exec tsc --noEmit --pretty false` when appropriate, plus dev-mode logs/UI observation when the change is runtime-facing.
...
- **Bundled plugin release rule**: When changing any bundled OpenClaw plugin under `resources/openclaw-plugins/*` (plugin entry files, manifest, package metadata, or runtime dependencies), bump that plugin's own `package.json` version before release and explicitly remind the release owner. Packaged apps refresh `~/.openclaw/extensions/<plugin>` at startup using version/content checks, but a version bump makes upgrade state obvious in logs and support diagnostics.
- **Formal Windows portable release**: Production Windows USB releases must use `.github/workflows/uclaw-portable-production.yml` from the latest `master`. The workflow mirrors the local release order: SignPath deep signing, Full regression against the signed ZIP, fresh-account registration Live on the protected `uclaw-release` runner, then OSS/feed/tag writes. Do not add a skip-tests or force-publish input. Runner credentials stay DPAPI-encrypted outside Git as documented in `.github/UCLAW_PORTABLE_RELEASE.md`.

...
function getAuthProfilesPath(agentId = 'main'): string {
  return join(resolveOpenClawStateDir(), 'agents', agentId, 'agent', AUTH_PROFILE_FILENAME);
}
...
async function readOpenClawJson(): Promise<Record<string, unknown>> {
  return (await readJsonFile<Record<string, unknown>>(resolveOpenClawConfigPath())) ?? {};
}

async function resolveInstalledFeishuPluginId(): Promise<string | null> {
  const extensionRoot = join(resolveOpenClawStateDir(), 'extensions');
...
export async function removeProviderFromOpenClaw(provider: string): Promise<void> {
  ...
  for (const id of agentIds) {
    const modelsPath = join(resolveOpenClawStateDir(), 'agents', id, 'agent', 'models.json');
    try {
...
export async function snapshotManagedAgentModelsFiles(): Promise<ManagedAgentModelsFilesSnapshot> {
  ...
  for (const agentId of agentIds) {
    const filePath = join(resolveOpenClawStateDir(), 'agents', agentId, 'agent', 'models.json');

...
import type { AgentProfile, AgentProfileDraft } from '@shared/types/agent';
import { getOpenClawConfigDir, resolveOpenClawWorkspacePath } from './paths';
...
export type AgentProfileInput = Partial<AgentProfile> & Partial<AgentProfileDraft>;
...
function normalizeProfile(
  ...
  return {
    ...
    avatarId: normalizeText(record.avatarId, existing?.avatarId || 'strategist'),
    createdAt: normalizeTimestamp(record.createdAt, existing?.createdAt || now),
    updatedAt: touchUpdatedAt
      ? now
      : normalizeTimestamp(record.updatedAt, existing?.updatedAt || now),
  };
}

/** Read every managed Agent profile from the active OpenClaw config directory. */
export async function readAgentProfiles(): Promise<Record<string, AgentProfile>> {
  try {
    const parsed = JSON.parse(await readFile(getProfileStorePath(), 'utf8'));
...
/** Normalize and persist one Agent profile. */
export async function upsertAgentProfile(agentId: string, input: AgentProfileInput): Promise<AgentProfile> {
  const profiles = await readAgentProfiles();
  const normalized = normalizeProfile(agentId, input, profiles[agentId], true);
  if (!normalized) throw new Error('Invalid agent profile');
  profiles[agentId] = normalized;
  await writeAgentProfiles(profiles);
  return normalized;
}

/** Delete only the requested managed Agent profile. */
...
function renderAgentsMarkerBlock(profile: AgentProfile): string {
  return [
    ...
    '## UClaw Agent Persona',
    '',
    `This workspace belongs to UClaw Agent "${profile.personaName}" (${profile.roleName}).`,
    `Read and follow \`${WORKSPACE_PROFILE_FILE}\` before responding in this workspace.`,
    'Keep responses aligned with the responsibilities, capabilities, boundaries, and workspace instructions in that profile.',
    AGENTS_MARKER_END,
  ].join('\n');
}
...
function upsertMarkedBlock(existing: string, block: string): string {
  ...
  const pattern = new RegExp(`${escapePattern(AGENTS_MARKER_START)}[\\s\\S]*?${escapePattern(AGENTS_MARKER_END)}`);
  if (pattern.test(existing)) return existing.replace(pattern, block);
  const prefix = existing.trimEnd();
  return `${prefix ? `${prefix}\n\n` : ''}${block}\n`;
}

/** Write the standalone profile and replace UClaw's managed AGENTS.md block. */
export async function writeAgentProfileWorkspaceFiles(
  agent: { id: string; workspace?: string },
  profile: AgentProfile,
): Promise<void> {
  const workspace = resolveOpenClawWorkspacePath(
    agent.workspace?.trim() || `~/.openclaw/workspace-${agent.id}`,
  );

...
/** Detect model/provider failure text that must not be parsed as a profile. */
...
/** Remove OpenClaw's common failure prefixes before presenting the error. */
...
/** Build the hidden-session prompt used to generate a structured Agent persona. */
export function buildAgentProfilePrompt(input: AgentProfileGenerationInput): string {
  ...
  return [
    ...
    `Output language: ${resolveLanguage(input.locale)}.`,
    '',
...
/** Build a deterministic persona only when Gateway history repeatedly times out. */
export function buildFallbackAgentProfile(input: AgentProfileGenerationInput): AgentProfileDraft {
...
/** Parse and validate the model's first complete JSON persona object. */

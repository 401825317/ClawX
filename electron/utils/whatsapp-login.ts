...
export class WhatsAppLoginManager extends EventEmitter {
    ...
    private async connectToWhatsApp(accountId: string): Promise<void> {
        ...
        try {
            ...
            const authDir = join(resolveOpenClawStateDir(), 'credentials', 'whatsapp', accountId);
    ...
    async stop(): Promise<void> {
        ...
        if (shouldCleanup && cleanupAccountId) {
            try {
                const authDir = join(resolveOpenClawStateDir(), 'credentials', 'whatsapp', cleanupAccountId);
                if (existsSync(authDir)) {
                    rmSync(authDir, { recursive: true, force: true });
                    ...
                    const parentDir = join(resolveOpenClawStateDir(), 'credentials', 'whatsapp');

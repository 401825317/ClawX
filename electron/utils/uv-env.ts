...
function buildClawXUvConfigToml(): string {
  return [
    '# This file is managed by UClaw.',
    '# It lets uv use UClaw-selected mirrors even when host exec filters UV_INDEX_URL.',
...
function ensureClawXUvConfigFile(): string | null {
  ...
  try {
    ...
    logger.warn('Failed to write UClaw uv config file:', err);
    return null;
  }
}

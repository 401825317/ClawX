...
function objectSection(value: unknown, field: string, errors: string[]): Record<string, unknown> {
...
function transform(value: unknown, field: string, errors: string[]): BlenderTransform | undefined {
  ...
  rejectUnknown(raw, ['location', 'rotation', 'scale'], field, errors);
  return {
...
export function validateSceneSpec(input: unknown): BlenderSceneSpecValidation {
  ...
  const projectRaw = objectSection(raw.project, 'project', errors);
  rejectUnknown(projectRaw, ['frameStart', 'frameEnd', 'fps'], 'project', errors);
  ...
  const renderRaw = objectSection(raw.render, 'render', errors);
  rejectUnknown(renderRaw, ['engine', 'width', 'height', 'samples', 'transparent'], 'render', errors);
  ...
  const deliverableRaw = objectSection(raw.deliverables, 'deliverables', errors);
  rejectUnknown(deliverableRaw, ['blend', 'glb', 'heroImage', 'turntable'], 'deliverables', errors);
  const budgetRaw = objectSection(raw.budgets, 'budgets', errors);
  rejectUnknown(budgetRaw, ['maxObjects', 'maxTriangles', 'maxTextureBytes', 'maxOutputBytes', 'maxRenderSeconds'], 'budgets', errors);
  const worldRaw = objectSection(raw.world, 'world', errors);
  rejectUnknown(worldRaw, ['color', 'strength'], 'world', errors);
  ...
  const spec: BlenderSceneSpec = {
    ...
    world: {
      color: rgba(worldRaw.color, 'world.color', errors, [0.035, 0.035, 0.05, 1]),
      strength: boundedNumber(worldRaw.strength, 'world.strength', errors, 0.25, 0, 5),
    },

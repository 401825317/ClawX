...
// Cancellation can change the shared snapshot while execute() is awaiting I/O.
function isCancelled(snapshot: BlenderJobSnapshot): boolean {
...
export class BlenderJobService {
  ...
  /** Stop queue consumption and terminate the Blender process owned by this app. */
  async shutdown(): Promise<void> {
    this.shuttingDown = true;
    this.queue.length = 0;
    if (this.activeChild) terminateBlenderProcess(this.activeChild);
  ...
  async create(request: BlenderJobRequest): Promise<{ job: BlenderJobSnapshot; idempotent: boolean }> {
    if (this.shuttingDown) throw new Error('Blender runtime is shutting down');
    await this.ensureInitialized();
  ...
  private async stageAssets(snapshot: BlenderJobSnapshot): Promise<BlenderSceneSpec> {
    ...
    const roots = [path.join(resolveOpenClawStateDir(), 'media'), snapshot.jobDir];

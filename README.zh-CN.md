...
> 托管视频的模型、模式、尺寸、时长及每个模型的默认值全部以后端能力合同为准。UClaw 会缓存最后一份有效合同以应对临时断网；用户未明确要求“最大/最长”时使用模型默认值；新机器离线且没有有效合同时直接关闭托管视频，不再猜测参数。
...
`VideoProject` 是每个最终视频请求的持久项目壳，无论它是单段还是多镜头：它记录创作约束、参考图血缘、稳定镜头身份、provider 尝试、QA 决策、字幕、旁白/音频策略、合成和交付。实际生成仍复用现有 `video_generate`；UClaw Host 对每段做可确定的媒体事实校验，并输出联系表供 Agent 做语义复核，不会谎称本地 OCR 或主观画面、音频判断已经完成。源片的实测尺寸必须满足已提交的生成合同；低规格输出会被阻断，不能靠放大冒充合格。每个项目都必须经过 Host 最终 QA，只有该流程能把同一个已验证成品标记为已合成或已交付。
...
便携包更新会先确认旧版完全退出，再事务性替换文件；若替换、回滚或新版窗口就绪校验失败，更新器会保留恢复备份和日志，而不会把半更新状态报成成功。macOS 便携包同时包含 Apple Silicon 与 Intel Mac 的更新辅助程序。

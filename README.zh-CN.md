...
会话渲染器现在只把 OpenClaw 和 Gateway 事实投影为一套事件驱动的线性 Timeline。时间相邻且兼容的工具调用会聚合展示，详情按需加载；可向用户展示的 thinking 默认折叠；产物、验证结果、审批和子任务仍作为独立信息项呈现。执行图不再常驻主信息流，需要时从**执行详情**打开；原始 canonical event 只在开发者诊断中可见。长历史采用虚拟化渲染，用户上滚后会脱离自动跟随，流式新内容不会把视口强行拉回底部。Phase 5 已删除 legacy/shadow 渲染器及运行时 rollout 开关；回滚通过回退代码或版本完成，不需要迁移 transcript 或已保存会话。

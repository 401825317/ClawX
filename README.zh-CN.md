...
发送 Sentry 事件前，客户端会删除已识别的凭证、Cookie、提示词、文件内容、请求体和响应体字段，过滤 Bearer/查询参数中的凭证，并遮蔽常见用户主目录路径。这是纵深防护，不是对任意内容的绝对隔离：放在未知字段中的敏感文本、UNC 路径或非标准用户路径仍可能残留；服务端隧道会转发已接收的 Sentry Envelope，不再进行第二次内容脱敏。

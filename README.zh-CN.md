...
默认会话渲染器现在会把 OpenClaw 和 Gateway 事实投影为事件驱动的线性 Timeline。时间相邻且兼容的工具调用会聚合展示，详情按需加载；可向用户展示的 thinking 默认折叠；产物、验证结果、审批和子任务仍作为独立信息项呈现。执行图不再常驻主信息流，需要时从**执行详情**打开；原始 canonical event 只在开发者诊断中可见。长历史采用虚拟化渲染，用户上滚后会脱离自动跟随，流式新内容不会把视口强行拉回底部。Phase 5 的一致性、E2E、产品兼容性和性能门槛全部通过前，旧投影仅作为迁移回滚路径保留。

运维可以通过 `client.rollouts.chatTimelineMode` 选择 `timeline`、`legacy` 或 `shadow`；启动环境变量 `CLAWX_CHAT_TIMELINE_MODE` 具有更高优先级，可用于紧急回滚。`shadow` 仍展示旧渲染器，但会在后台归约 canonical event，只在历史检查点和权威 run 终态做语义对比。对比诊断按会话限制容量，不保存消息正文、工具结果或凭证。

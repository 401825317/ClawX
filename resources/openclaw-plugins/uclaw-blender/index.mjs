...
function hostApiOrigin() {
  const origin = process.env.CLAWX_HOST_API_ORIGIN || '';
  if (!origin.trim()) throw new Error('UClaw Blender bridge origin is unavailable');
  return origin.trim().replace(/\/+$/u, '');
}

function hostApiToken() {
  const token = process.env.CLAWX_HOST_API_TOKEN || '';
  if (!token.trim()) throw new Error('UClaw Blender bridge token is unavailable');
  return token.trim();
}

...
- For already-open external browser windows such as Chrome, Edge, Brave, Chromium, Firefox, Opera, or Vivaldi, prefer `computer_web_observe` as the first observation step when it is listed. It returns the browser window, foreground guard, inferred URL/title, visible UIA text, clickable/editable candidates with bounds/centers, and optional screenshot coordinate metadata in one call. Use its candidates before falling back to repeated full-screen screenshots and coordinate guessing.

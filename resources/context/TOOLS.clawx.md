...
- If a shell command must create a temporary screenshot or image for a later vision/image/local-media read, save it under an OpenClaw media/workspace directory such as `~/.openclaw/media/outbound/`, not directly under `/tmp/*.png`. Local media tools reject bare system-temp image paths outside the managed allowlist.

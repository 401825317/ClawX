...
- If `profile: "user"` fails with `DevToolsActivePort` or `Could not connect to Chrome`, stop retrying that profile. Use the managed `openclaw` browser profile only if it can satisfy the task; otherwise report the missing logged-in session or desktop capability.
- Do not use the `browser` tool for native desktop apps, file pickers, OS dialogs, QR/captcha/2FA blockers, or existing non-Chrome user browser windows. Continue only when a concrete reliable connector for that surface is listed; otherwise report the blocker.
...
7. If the page shows login, permission, captcha, QR code, 2FA, or an external app handoff, report the blocker unless a concrete reliable connector for that surface is listed.

...
Status: current compatibility reference, reviewed 2026-08-08.
...
3. Failed live-Turn prefix reconciliation. After the matching ACP prompt explicitly fails, one immediate transcript read may extend the sole existing Assistant text segment in that exact user Turn from the delivered prefix to the longer failed Assistant text persisted by OpenClaw. The transcript message must itself carry an error stop reason or error message. This path cannot create a segment, shorten or replace divergent text, cross a user boundary, retry, or run after cancellation of that Turn or replacement of its session generation. Navigation and a later unrelated prompt do not transfer its owner or make it write into the visible conversation.

The media content exceptions use bounded transcript operations keyed by exact session, generation, and live user message. They keep projected state in memory and reject stale or ambiguous evidence, but navigation alone is not staleness. Operations for different conversations or Turns remain independent, and retries for one operation execute serially so a later attempt cannot invalidate an older resolution still in flight. Existing-session load reads at most 1000 recent transcript messages. An ordinary successful live prompt performs one immediate read and begins its bounded retry schedule after 1500 milliseconds. An `image_generate` task recorded for that same live prompt extends the operation through bounded backoff while waiting for its completion artifact. OpenClaw 6.10 may finish a detached media completion agent with an ordinary terminal runtime event on the requester session instead of a task-scoped event. Such an event may re-arm only the bounded supplement for media tasks that remain pending in that exact Turn; completed tasks, cancellation of that Turn, superseded generations, and expired operations cannot trigger another read. A terminal `video_generate` task recorded for the same live prompt starts a separate bounded backoff for only that original user-turn identity because OpenClaw persists the final local `MEDIA:` path after the detached completion event. It stops as soon as the expected number of completed tasks resolves to local `video/*` identities, or on invalidation or retry-window exhaustion. Failed live-Turn prefix reconciliation performs only its one immediate read after failure and shares the same operation guard. These exceptions must be removed when the distributed OpenClaw ACP adapter emits the equivalent standard content.
...
3. Requires the exact owning session generation and accepted Turn operation to remain valid; another visible session does not invalidate it.
...
Every asynchronous result is valid only for the same owning session key, ACP generation, supplement operation, attempt, and, for live recovery, user-turn identity. Navigation and an unrelated later prompt do not transfer or cancel that ownership. Cancellation of the owning Turn, a superseded generation, an expired operation, or a mismatched evidence anchor prevents stale mutation. Attachment projections commit through the session timeline coordinator and merge monotonically: pending or unavailable evidence may become available, while an available result cannot regress because of a delayed pending or failure replay.
...
OpenClaw returns an asynchronous `video_generate` task id before provider polling finishes. Renderer keeps only the pending task ids for the affected live session, shows the localized composer status while one remains, and blocks another send without disabling draft editing. A matching inter-session completion event or terminal background runtime/Gateway event clears the task; a timeout bounded by the managed video timeout prevents a permanently locked composer when completion delivery is lost. Pending task state never appends timeline content or changes event order. A proven terminal success may trigger only the bounded original-Turn media supplement described above. A proven terminal failure may add one generic, Turn-scoped `video-generation` compatibility projection; ordinary ACP replay remains the history base, and only that marked failure projection may be restored idempotently onto its matching replay Turn. Neither path starts general history polling, recovers ordinary messages, exposes provider internals, or reorders ACP events.

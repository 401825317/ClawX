...
Status: current architecture reference, reviewed 2026-08-08.
...
Renderer-visible session identity is the OpenClaw Gateway session key. Main may hold a different ACP session id returned by `newSession`; it rewrites downstream routing to the matching Gateway session key. Loads on the shared ACP connection are serialized. A routing envelope carries the session key and the Main-owned generation token for the matching load or live prompt. Renderer uses a separate local request sequence to reject stale load completions; preparing a local-only session must not advance the ACP generation. A session timeline coordinator owns the exact `sessionKey + generation` snapshot, serializes active and retained-background commits, and advances a per-record monotonic revision. Renderer ignores updates, permission requests, and asynchronous hydration results whose identity matches neither the selected session nor a retained owner. Generation is an in-memory race token rather than a durable sequence; Main may restore the previous value when a load fails, so code must compare it together with session and current-operation state rather than assume global monotonicity.

While `session/prompt` is pending, Main retains a bounded session-id routing context and Renderer retains that prompt's reduced timeline and original client-observed turn start in memory. This lets another page or conversation be viewed without dropping the original stream or resetting elapsed time. Returning to the live conversation reactivates its existing ACP context and restores the memory snapshot without calling `session/load`; updates received during the handoff are still generation-filtered. Prompt settlement releases the Main routing context. If that Turn accepted asynchronous image, video, or explicit `MEDIA:` work, Renderer retains its session-scoped owner through terminal delivery. Navigation changes only the visible session, and a later prompt creates another Turn operation instead of cancelling the earlier delivery.

Renderer keeps at most three settled, not-yet-consumed background snapshots and three unretained coordinator records. Active sends, pending media, transcript supplements, attachment hydration, and other explicitly retained owners are exempt until they settle; eviction never cancels accepted in-flight delivery.

Returning during that bounded background phase still performs ordinary ACP replay. The replay remains authoritative for messages and process ordering; Renderer carries forward only items already marked as image-generation or OpenClaw MEDIA compatibility projections. Because ACP ids are not durable across loads, a projection is restored only when normalized user prompt text plus occurrence from the tail identifies one semantic Turn. Synthetic media is inserted at that Turn's end, native assistant media is restored only onto one same-caption reply, and deduplication by evidence and opaque media identity remains Turn-scoped. Every attachment reference is rebound to the new session generation. Unmatched projections are left for the normal bounded transcript supplement. This is operation state, not a second history ledger, and it is never persisted.

`messageId` and `toolCallId` are opaque identities within one loaded timeline. They are not durable UI identities across loads. When OpenClaw 6.10 omits a message id, Renderer assigns a role-local monotonic fallback identity that is independent of item count; compatibility overlays do not advance it or close its open segment. Timeline sequence values and DOM anchors are also local to the active snapshot.
...
There are exactly four approved transcript-derived content supplements. ClawX may recover asynchronous image-generation completions with proven `image_generate` context, recover explicit line-leading assistant `MEDIA:` attachment directives omitted by OpenClaw ACP, settle the last strict-prefix Assistant segment in an exactly aligned successful media Turn, and extend one already-visible Assistant text segment after ACP reports that exact prompt failed. The successful media exception accepts final text only from a model-authored Assistant message with a validated `MEDIA:` line and no failure marker, excludes `gateway-injected` mirrors and failed records, and cannot insert a message, replace divergent text, select an earlier segment, or cross a user boundary. The failed-turn exception requires an unambiguous current user identity, a transcript Assistant message explicitly marked failed, and transcript text that is a strict prefix-preserving extension of the one existing live segment; it cannot insert a message or replay history. All four are bounded, marked, memory-only projections. Separately, Main may extract metadata-only whole-turn timing because ACP replay omits original timestamps. Renderer can attach that timing only to an unambiguously matched ACP turn; it cannot otherwise reconstruct ordinary assistant text, thoughts, tool cards, plans, permissions, file activity, or missing turns. See `harness/reference/acp-generated-media-and-diagnostics.md#bounded-transcript-exceptions` for the content compatibility grammar and timing boundary.
...
Renderer keeps attachment references and compatibility projections in the owning in-memory timeline. Main owns ACP session and relative-path context and resolves, reads, and opens every attachment against the exact session and generation. Resolution commits through the same session coordinator even while another conversation is visible. Attachment state is monotonic: a later success may upgrade pending or unavailable evidence, while delayed pending or failure results cannot downgrade an available attachment. Existing regular files may resolve outside the active workspace, but a prior resolution is not reusable authorization and Renderer cannot supply a replacement execution cwd. Native ACP evidence wins when it resolves to the same identity as compatibility evidence. The complete authorization and URI boundary is documented in `harness/reference/acp-attachment-access-control.md`.

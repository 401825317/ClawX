...
Returning during that bounded background phase still performs ordinary ACP replay. The replay remains authoritative for messages and process ordering; Renderer carries forward only items already marked as image-generation or OpenClaw MEDIA compatibility projections. A projection is restored only at a replay anchor that survived the load, is deduplicated by evidence and opaque media identity, and has every attachment reference rebound to the new session generation. Unanchored projections are left for the normal bounded transcript supplement. This is operation state, not a second history ledger, and it is never persisted.

...
Portable upgrades and computer changes may leave the ACP database with a physical cwd from an older USB drive letter, macOS volume path, legacy `UClawData` state directory, or local `UClawRuntime` profile. Main maps only a path that is provably the default workspace for the active portable identity back to the stable logical default. Custom workspace paths remain authoritative and are never guessed or relocated.

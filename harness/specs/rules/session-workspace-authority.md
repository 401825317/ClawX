...
In portable mode, a persisted ACP cwd that can be proven to be the canonical default workspace for the same portable identity is normalized back to `~/.openclaw/workspace` before it reaches Renderer. This recovery covers the legacy USB state layout and a prior computer's isolated `UClawRuntime` profile on Windows and macOS. It does not relocate arbitrary user-selected folders or paths belonging to another portable identity.

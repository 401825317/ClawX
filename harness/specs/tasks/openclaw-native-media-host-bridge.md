...
expectedUserBehavior:
  ...
  - `video_generate action=list` exposes only models supported by registered video capabilities and allowed by provider configuration. Shared chat entries do not become video candidates merely because chat and video share a provider namespace. The local timeline renderer remains a fallback after real video candidates are unavailable, not a replacement for provider discovery.

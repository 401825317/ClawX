...
acceptance:
  ...
  - The managed `openai/smart-latest` model declares native `xhigh` support in both catalog compatibility and its top-level thinking-level map, so an `xhigh` Agent turn reaches the final OpenRouter payload as `reasoning.effort=xhigh` instead of being clamped to `high`.

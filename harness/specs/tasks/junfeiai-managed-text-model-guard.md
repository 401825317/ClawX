...
intent: Ensure the managed `lingzhiwuxian` provider only sends text models exposed by JunFeiAI client-config, healing stale session, agent, and runtime defaults before chat requests reach OpenClaw.
touchedAreas:
...
expectedUserBehavior:
  - A user whose old conversation or agent stored `lingzhiwuxian/gpt-5.5` is silently moved back to the server-provided text model such as `lingzhiwuxian/smart-latest`.
...
- The managed fallback is `client.modelOptions.text.defaultModel` when present
  ...
  `smart-latest`.
- Only `lingzhiwuxian/*` refs are rewritten. Other provider refs remain user
  controlled.

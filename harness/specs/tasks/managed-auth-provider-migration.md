...
expectedUserBehavior:
  ...
  - While login or registration is committing credentials and initializing the managed runtime, the account form is replaced by localized account, Gateway, and finalization stages driven by the existing Gateway state, with elapsed time and a first-start delay hint instead of a fabricated percentage; a failed attempt restores the form.
  ...
  - After a successful login or registration commits its managed credentials, the transaction owner starts Gateway with its lease even when no Gateway was active before authentication. A new PID in `starting` or `running` is accepted while `gateway.ready` is pending; if the lease-owned start fails, authentication retries once after releasing the lease so normal automatic reconnect can continue.
...
acceptance:
  ...
  - Successful authentication replaces stale or user-authored data that collides with either reserved managed identity, removes dynamically discovered historical UClaw Relay aliases, and installs both managed accounts plus matching `openai:default` and `lingzhiwuxian:default` OpenClaw auth profiles. It preserves generic `openai-codex` OAuth state unless that record is independently identified as a UClaw Relay by its current content, and does not physically delete unrelated custom OpenAI-compatible, DeepSeek, other third-party, or internal image/video Provider accounts, secrets, runtime entries, or model metadata.
  ...
  - The persistent mutation marker is written after Gateway quiescence but before `snapshot()` and therefore before Provider Store migration or any other possible credential write. The complete snapshot freezes Provider accounts/default keys, Secret slots, verification/device state, runtime config, every Agent `models.json`, and every Agent auth-profile JSON byte sequence and SQLite primary row. Its managed-ID set is the union of the two reserved IDs plus IDs dynamically identified from Provider Store, runtime, and all frozen Agent model snapshots by managed ownership markers or the current UClaw Relay fingerprint.
  - The lifecycle order is lease acquisition, Gateway stop plus in-flight/late-start drain, port-free proof, persistent marker, complete snapshot, transactional commit or CAS-protected exact rollback / staged cleanup, marker clear only after completeness, owner-only start after successful authentication, healthy new-PID verification, then lease release. `starting` with a new PID is a healthy launch while readiness is pending. If that start fails after a committed authentication generation, release the lease before one immediate recovery start so Gateway Manager reconnect remains enabled. A late child that cannot be terminated makes quiescence fail before marker or credentials are written.
  ...
  - Pending login and registration replace editable credentials with an accessible live status that advances from account/configuration through actual Gateway startup to Provider refresh, displays elapsed seconds, adds a localized slow-start hint after 15 seconds, and restores the form on failure without adding a Renderer-owned Gateway lifecycle call or synthetic progress percentage.
...
- Managed authentication owns the reserved `openai` and `lingzhiwuxian` Provider
  ...
  removes real-OpenAI conflicts and dynamically identified UClaw Relay aliases, then installs the
...
- The complete snapshot covers Provider/default/secret records, verification cache,
  ...
  primary rows. Its dynamic managed-ID union comes from the two reserved IDs plus Provider
  Store, runtime, and Agent model records identified by managed markers or the current Relay fingerprint. Rollback
...
- Clear the persistent marker only after commit, rollback, or staged cleanup is complete.
  After successful authentication, only the lease owner may perform the initial start and
  must verify a healthy new PID in `starting` or `running` before releasing the lease. If
  that start fails, release the lease before one immediate recovery start so normal
  automatic reconnect remains enabled. Ordinary start, restart, reload, debounced work,
  and reconnect fail closed while a lease or marker exists.
...
- Treat `openai-codex` as a generic OpenAI Codex OAuth compatibility alias rather than a
  UClaw-managed identity. UClaw must not claim it solely by id; generic OAuth alias
  normalization, active-provider detection, and legacy runtime migration remain supported.
  Newly committed UClaw state uses canonical provider/account `openai`, compatibility
  provider/account `lingzhiwuxian`, auth account `uclaw-auth`, identical resolved model sets,
  default `openai/<resolved-default>`, and canonical protocol `openai-responses`.
